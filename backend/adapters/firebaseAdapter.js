...
class ApiAdapter {
  ...
  async getBooks(limit = 50, offset = 0) {
    try {
      const response = await axios.get(
        `${this.baseURL}/books?limit=${limit}&offset=${offset}`
      );
      if (response.data && response.data.success) {
        return response.data.data;
      }
  ...
  async deleteBook(bookId) {
    try {
      const headers = await this.getHeaders();
      const response = await axios.delete(`${this.baseURL}/books/${bookId}`, {
        headers,
      });
      if (response.data && response.data.success) {
        return true;
  ...
  async searchBooks(query) {
    try {
      const response = await axios.get(
        `${this.baseURL}/books/search?q=${encodeURIComponent(query)}`
      );
      if (response.data && response.data.success) {
        return response.data.data;
      }
  ...
  async createTrend(trendData) {
    try {
      const headers = await this.getHeaders();
      const response = await axios.post(`${this.baseURL}/trends`, trendData, {
        headers,
      });
      if (response.data && response.data.success) {
        return response.data.data;
  ...
  async getTrends() {
    try {
      ...
      if (response.status === 404) {
        throw new Error(
          "Gündem konuları servisi bulunamadı. Lütfen sistem yöneticinize başvurun."
        );
      }
      throw new Error(response.data.message || "Gündem konuları alınamadı");
    } catch (error) {
      console.error("Gündem konuları hatası:", error.message);
      throw new Error(
        "Gündem konuları alınamadı. Lütfen daha sonra tekrar deneyin."
      );
    }
  }
  ...
  async getTrend(trendId) {
    try {
      ...
      throw new Error(
        response.data.message || "Gündem konusu bilgileri alınamadı"
      );
    } catch (error) {
  ...
  async deleteTrend(trendId) {
    try {
      const headers = await this.getHeaders();
      const response = await axios.delete(`${this.baseURL}/trends/${trendId}`, {
        headers,
      });
      if (response.data && response.data.success) {
        return true;
